...
rpal_dir=$"C:\Users\comcc_000\Desktop\UF Spring 2016\Programming Language Principles\RPAL\rpal"
...
Test_Helper() { # Compile and run the specified program; pass along any additional parameters; log everything 
    ...
    then
        (cd "$rpal_dir"; ./rpal tests/$1) 
    else 
        (cd "$rpal_dir"; echo "${@:2}" | ./rpal tests/$1)  
    fi                     
}
